...
int open_codec(AVFormatContext* fmt_ctx,enum AVMediaType type, AVCodecContext** codec_ctx, int* stream_index)
{
	...
	if (ret < 0) {
		av_log(NULL, AV_LOG_ERROR, "av_find_best_stream failure! Type: %s\n", av_get_media_type_string(type));
		return ret;
	}
	index = ret;
	...
	if (!codec) {
		av_log(NULL, AV_LOG_ERROR, "avcodec_find_decoder failure! Codec Id: %d\n", st->codecpar->codec_id);
		return -1;
	}

	AVCodecContext* ctx = avcodec_alloc_context3(codec);
	if (!ctx) {
		av_log(NULL, AV_LOG_ERROR, "avcodec_alloc_context3 failure!\n");
		return -1;
	}
