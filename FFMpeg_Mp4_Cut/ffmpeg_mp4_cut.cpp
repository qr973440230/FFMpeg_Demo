...
int main(int argc, char *argv[]) {
    ...
    ret = avformat_alloc_output_context2(&ofmt_ctx, nullptr, nullptr, dst);
    if (ret < 0) {
        av_log(nullptr, AV_LOG_ERROR, "Cannot alloc output context!\n");
    ...
    for (unsigned int i = 0; i < nb_streams; i++) {
        ...
        if (!out_stream) {
            av_log(nullptr, AV_LOG_ERROR, "Cannot new stream!\n");
        ...
        if (ret < 0) {
            av_log(nullptr, AV_LOG_ERROR, "Cannot copy parameters!\n");
            avformat_free_context(ofmt_ctx);
            avformat_close_input(&fmt_ctx);
            return ret;
        }
    ...
    if (!(ofmt_ctx->flags & AVFMT_NOFILE)) {
        ...
        if (ret < 0) {
            av_log(nullptr, AV_LOG_ERROR, "Cannot open file! File: %s!\n", dst);
            avformat_free_context(ofmt_ctx);
            avformat_close_input(&fmt_ctx);
            return ret;
        }
    ...
    if (ret < 0) {
        av_log(nullptr, AV_LOG_ERROR, "Cannot seek to %llu!\n", from_seconds);
        avformat_free_context(ofmt_ctx);
        avformat_close_input(&fmt_ctx);
        return ret;
    }
    ...
    while (true) {
        ...
        if (ret < 0) {
            av_log(nullptr, AV_LOG_ERROR, "Cannot interleaved write frame!\n");
            av_packet_unref(&pkt);
            break;
        }
        av_packet_unref(&pkt);
